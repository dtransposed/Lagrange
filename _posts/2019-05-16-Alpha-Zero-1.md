...
This first entry will discuss the concepts of __Upper Confidence Bound__ (UCB), __Monte Carlo Search Trees__ (MCST) and __Upper Confidence Bound for Search Trees__ (UCT).  I will not only discuss the concepts but also implement the algorithms. This is because my final goal is not only to understand the mathematics behind concepts but also be able to express them in form of a code. I also assume that the reader has some prior knowledge pertaining the fundamentals of reinforcement learning.
