...
GANs have undoubtedly proven how powerful the deep neural networks are. There is something beautiful about the way a machine learns about creating stunning, high resolution images as if it understood the world like we do. But, just like the rest of those wonderful statistical models, their biggest flaw is the lack of interpretability. 
This research makes a very important step towards understanding GANs. Let’s say that we want to find out, which units in the generator are 'responsible' for generation of certain objects which belong to class $$c$$. The authors claim, that we can inspect a layer of the generator and classify its units into two groups: the ones which are indifferent to the generation of class $$c$$ objects in the image and the ones which are sensitive to the presence of class $$c$$ objects in the generated image. The authors search for those particular 'class-related' units, by using two steps: dissection and intervention. Additionally, they also claim that this is the first work, which provides systematic analysis for understanding of the GANs'internal mechanisms.

### The method:

A generator can be viewed as a mapping from a latent vector $$\textbf{z}$$ to an generated image $$\textbf{x}$$. Our goal is to understand the $$\textbf{r}$$, an internal representation, which is an output from particular layer of the generator $$G$$. We would like to inspect  $$\textbf{r}$$ closely, with respect to objects of the class $$c$$. We know that $$\textbf{r}$$ somehow has internally encoded the mechanism of class $$c$$ object generation and we want to understand - how? The authors propose, that there is a way to extract from $$\textbf{r}$$ those units, which are being responsible for generation of class $$c$$ objects.

Here, U set contains U and U/ is the set of all units in the particular layer, and P are pixel locations.

The question now is, how to perform this separation? The authors propose two ideas which give as an understanding of the internals of GAN “black-box”. Those are “dissection” and “intervention”.
structure

Dissection - we identify the classes, which have an explicit representation in r by comparing two images. We obtain the first image by computing x, and then running in through a semantic segmentation network, which would return pixel locations s_c(x) corresponding to the class of interest (trees). The second image is being generated in the following way. We take r_u,P , upsample it so it matches the dimension of sc_(x) and then threshold it to check which pixel localization is being “excited” by this particular unit. Finally we compute spatial agreement between both outputs. The higher the intersection over union (IoU), the better is our split. By performing this operation for every unit, we should eventually find out, which classes have an explicit representation in the structure of r.

Intervention - at this point we have identified the relevant classes. Now, we split the r into .. and .. and attempt to find the best separation. This means that on one hand we ablate (supress) ...  units, hoping that the class of interest would disappear from the generated image. On the other hand, we amplify they influence of … on the generated image to learn how much this particular set of units contributes to the presence of the class of interest. Finally, we segment out the class c from both images and make a comparison. The less agreement between the semantic maps the better. This means that on one image we have completely ‘tuned out’ the influence of trees, while the second image contain solely leaves.
