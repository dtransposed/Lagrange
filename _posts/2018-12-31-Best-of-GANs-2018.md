...
Now, a latent vector $$textbf{w}$$ sampled from intermidiate latent space is being fed into the block 'A' (learned affine transform) and translated into a style $$\textbf{y} =(y_s,y_b)$$. The style is finally injected into the synthesis network through adaptive instance normalization (AdaIN) at each convolution layer. The AdaIN operation is defined as:

$$AdaIN(textbf{x_i},textbf{y}=$$
