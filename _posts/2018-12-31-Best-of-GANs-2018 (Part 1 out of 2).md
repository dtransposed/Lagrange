...
This research makes a very important step towards understanding GANs. It allows us to find out, which units in the generator are "responsible" for generation of certain objects which belong to some class $$c$$. The authors claim, that we can inspect some layer of the generator and find a subset of it's units which cause the generation of $$c$$ objects in the generated image. The authors search for the set of "causal" units for each class by introducing two steps: dissection and intervention. Additionally, this is probably the first work, which provides systematic analysis for understanding of the GANs' internal mechanisms.
