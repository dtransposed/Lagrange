...
![alt text](/assets/4/6.png)
{: refdef}
<em> a) Images of churches generated by the Progressive GAN, b) Given the pre-trained Progressive GAN we identify the units responsible for tree generation, c) we can either supress those units to "erase" trees from images..., d) amplify class trees in the image</em>
...
{: refdef}
<em> a) for debugging purposes, we can identify those units which are introducing artifacts, b) and c) and turn them off to get "repair" the GAN </em>
