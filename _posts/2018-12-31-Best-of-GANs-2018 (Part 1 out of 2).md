...
image: gan.jpg
---
This year I had a great pleasure to be involved in a research project, which required me to read a substantial number of publications from the domain of deep learning for computer vision. It allowed me to take a deep dive into the domain and I was amazed by the progress done in the last 2-3 years. It is truly exciting and motivating how all the different subfields such as image impainting, adversarial examples, super-resolution or 3D reconstruction have greatly benefited from the recent research. However, there is one invention, which has earned truly massive amounts of hype (in my humble opinion - definitely for a reason)- __Generative Adverserial Networks__ (GANs). I can agree that those models are one of the most fascinating inventions in computer vision and I am always on a lookout for some new GAN ideas.

Inspired by this [very modest reddit discussion](https://www.reddit.com/r/MachineLearning/comments/a8th4o/d_what_are_best_papers_regarding_gans_for_2018/), I have decided to make a quick overview of the most interesting publications from 2018 regarding GANs. In this first chapter I will discuss three research papers. A side note, if you are interested in older GAN papers, [this article](https://medium.com/nurture-ai/keeping-up-with-the-gans-66e89343b46) may be helpful. One of the papers mentioned by Rowen Lee even made it to my top list.

## Table of Contents

1. GAN Dissection: Visualizing and Understanding Generative Adversarial Networks - given amount of hype around GANs, it is obvious that this technology, sooner or later, will be used commercially. However, because we know so little about their inner mechanics (a black box problem), I think taht it is still difficult to create a reliable product. This work takes a huge leap towards the future, where we are able to truly control GANs. Please check out this [great interactive demo](https://gandissect.csail.mit.edu/), the results are stunning!
2. A Style-Based Generator Architecture for Generative Adversarial Networks - it is no suprise that NVIDIA research team constantly comes up with trail-blazing ideas (great [image impainting paper from 2018](https://www.youtube.com/watch?v=gg0F5JjKmhA), quite recent demo of using [neural networks for graphics rendering](https://www.youtube.com/watch?time_continue=2&v=ayPqjPekn7g)). This paper is no exception. The video which shows the result of this research is simply mesmerising.
3. what I love about this paper is clever idea and great readability. Evolutionary algorithms together with GANs are something which is bound to be cool.
...
A generator can be viewed as a mapping from a latent vector $$\textbf{z}$$ to an generated image $$\textbf{x}$$. Our goal is to understand the $$\textbf{r}$$, an internal representation, which is an output from particular layer of the generator $$G$$. We would like to inspect  $$\textbf{r}$$ closely, with respect to objects of the class $$c$$. We know that $$\textbf{r}$$ somehow has internally encoded the mechanism of $$c$$ objects generation and we want to understand - how? The authors propose, that there is a way to extract from $$\textbf{r}$$ those units, which are being responsible for generation of class $$c$$ objects, and separate them from the rest of the units.
...
__Intervention__ - at this point we have identified the relevant classes. Now, attempt to find the best separation for every class. This means that on one hand we ablate (supress) unforced units, hoping that the class of interest would disappear from the generated image. On the other hand, we amplify they influence of causla units on the generated image to learn how much they contribute to the presence of the class of interest. Finally, we segment out the class $$c$$ from both images and make a comparison. The less agreement between the semantic maps, the better. This means that on one image we have completely ‘tuned out’ the influence of trees, while the second image contains solely a jungle.
